...
describe('detectChanges', () => {
  ...
  it('should detect added properties', () => {
    ...
    expect(changes).toEqual([{ type: 'add', path: 'b', newValue: 2 }]);
  });

  it('should detect removed properties', () => {
    const changes = detectChanges({ a: 1, b: 2 }, { a: 1 });
    expect(changes).toEqual([{ type: 'remove', path: 'b', oldValue: 2 }]);
  });
  ...
  it('should detect array value changes', () => {
    const changes = detectChanges({ items: [1, 2, 3] }, { items: [1, 2, 4] });
