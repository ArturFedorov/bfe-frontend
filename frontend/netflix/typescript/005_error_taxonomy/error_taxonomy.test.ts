...
type _cases = [
  ...
  Expect<
    Equal<ErrorByKind<'auth'>, { kind: 'auth'; reason: 'expired' | 'missing' }>
  >,
  Expect<
    Equal<ErrorHandlerMap['network'], (error: ErrorByKind<'network'>) => string>
  >,
];
...
type ExtendedHandlerMap = {
  [K in ExtendedError['kind']]: (
    error: Extract<ExtendedError, { kind: K }>,
  ) => string;
};
...
describe('describeError', () => {
  it('describes network errors', () => {
    ...
    ).toBe('Network 503 on /v1/partners');
  });
  ...
  it('describes auth errors for both reasons', () => {
    expect(describeError({ kind: 'auth', reason: 'expired' })).toBe(
      'Auth error: token expired',
    );
    expect(describeError({ kind: 'auth', reason: 'missing' })).toBe(
  ...
  it('stringifies unknown causes', () => {
    ...
    expect(describeError({ kind: 'unknown', cause: 42 })).toBe(
      'Unknown error: 42',
    );
  });
});
